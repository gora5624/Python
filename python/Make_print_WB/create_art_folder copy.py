...
def main(path_list_stuff, model_name):
    for line in read_xlsx(path_list_stuff):
        for file in os.listdir(os.path.join(r'D:\printsPy', model_name)):
            fullName = model_name + ' ' + file[0:-4]
            if line['Название 1С'].replace('\xa0', ' ') in fullName:
