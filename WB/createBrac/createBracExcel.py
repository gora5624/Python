...
pathToFileWithBarcodes = r'\\192.168.0.33\shared\_Общие документы_\Заказы вайлд\BrakHelp\ШК для брака.xlsx'
counter = r'\\192.168.0.33\shared\_Общие документы_\Заказы вайлд\BrakHelp\counter.txt'
...
while True:
    ...
    caseListTMP.to_excel(os.path.join(os.path.join(r'\\192.168.0.33\shared\_Общие документы_\Заказы вайлд\Браки', branFileName.format(str(count),curData, curTime))), index=False)
    print('Файл создан в \\192.168.0.33\shared\_Общие документы_\Заказы вайлд\Браки')
