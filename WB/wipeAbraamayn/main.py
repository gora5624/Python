...
# wipeListDF = pd.DataFrame(pd.read_excel(r"F:\Первая возна на зануление.xlsx")).to_dict('records')
# with open(r'D:\Python\WB\wipeAbraamayn\wipeListDict.pkl', 'wb') as wipeListDictFile:
#     pickle.dump(wipeListDF, wipeListDictFile)
#     wipeListDictFile.close()
with open(r'D:\Python\WB\wipeAbraamayn\wipeListDict.pkl', 'rb') as wipeListDictFile:
...
def wipeData(wipeListDF):
    ...
    chrtIDList = []
    try:
        with open(r'D:\Python\WB\wipeAbraamayn\chrtIDList.pkl', 'rb') as chrtIDListFile:
            chrtIDListDone = pickle.load(chrtIDListFile)
            chrtIDListFile.close()
    except:
        chrtIDListDone = []
    try:
        with open(r'D:\Python\WB\wipeAbraamayn\chrtIDListErrors.pkl', 'rb') as chrtIDListErrorsFile:
            chrtIDListErrors = pickle.load(chrtIDListErrorsFile)
            chrtIDListErrorsFile.close()
    except:
        chrtIDListErrors = []
    try:
        with open(r'D:\Python\WB\wipeAbraamayn\badNmIDlist.pkl', 'rb') as badNmIDlistFile:
            badNmIDlist = pickle.load(badNmIDlistFile)
            badNmIDlistFile.close()
    except:
        badNmIDlist = []
    # dfTMP = pd.DataFrame({'chrtIDListDone':chrtIDListDone}).to_excel(r'D:\Python\WB\wipeAbraamayn\chrtIDListDone.xlsx', index=False)
    # # dfTMP = pd.DataFrame({'badNmIDlist':badNmIDlist}).to_excel(r'D:\Python\WB\wipeAbraamayn\badNmIDlist.xlsx', index=False)
    # dfTMP = pd.DataFrame({'chrtIDListErrors':chrtIDListErrors}).to_excel(r'D:\Python\WB\wipeAbraamayn\badNmIDlist.xlsx', index=False)
    for line in wipeListDF:
        # if (line['chrtID'] not in  chrtIDListDone) and line['nmID'] not in badNmIDlist:
        if (line['chrtID'] in chrtIDListErrors)  and (line['nmID'] not in badNmIDlist):
            sizes = [
                {
                    "chrtID": line['chrtID'],
                    #"price": 590,
        ...
        if len(json) >= 20:
            try:
                ...
                with open(r'D:\Python\WB\wipeAbraamayn\chrtIDListErrors2.pkl', 'wb') as chrtIDListErrorsFile:
            ...
            if r.status_code ==200:
                ...
                with open(r'D:\Python\WB\wipeAbraamayn\chrtIDList.pkl', 'wb') as chrtIDListFile:
                ...
                if 'Invalid nmID' in r.text:
                    ...
                    with open(r'D:\Python\WB\wipeAbraamayn\badNmIDlist.pkl', 'wb') as badNmIDlistFile:
...
def changePrice(wipeListDF):
    try:
        with open(r'D:\Python\WB\wipeAbraamayn\priceDoneList.pkl', 'rb') as priceDoneListFile:
            priceDoneList = pickle.load(priceDoneListFile)
            priceDoneListFile.close()
    except:
        priceDoneList = []
    try:
        with open(r'D:\Python\WB\wipeAbraamayn\priceErrorsList.pkl', 'rb') as priceErrorsListFile:
        ...
        if line['nmID'] not in priceDoneList:
            ...
            if len(jsonPrice)>=1000:
                try:
                    ...
                    with open(r'D:\Python\WB\wipeAbraamayn\priceErrorsList.pkl', 'wb') as priceErrorsListFile:
                        pickle.dump(priceErrorsList, priceErrorsListFile)
                        priceErrorsListFile.close()
                    continue
                if r1.status_code == 200:
                    with open(r'D:\Python\WB\wipeAbraamayn\priceDoneList.pkl', 'wb') as priceDoneListFile:
...
def wipeMediafiles(wipeListDF):
    try:
        with open(r'D:\Python\WB\wipeAbraamayn\mediaDoneList.pkl', 'rb') as mediaDoneListFile:
            mediaDoneList = pickle.load(mediaDoneListFile)
            mediaDoneListFile.close()
    except:
        mediaDoneList = []
    try:
        with open(r'D:\Python\WB\wipeAbraamayn\mediaErrorsList.pkl', 'rb') as mediaErrorsListFile:
    ...
    for line in wipeListDF:
        if line['vendorCode'] not in mediaDoneList:
            ...
            try:
                ...
                with open(r'D:\Python\WB\wipeAbraamayn\mediaErrorsList.pkl', 'wb') as mediaErrorsListFile:
                        pickle.dump(mediaErrorsList, mediaErrorsListFile)
                        mediaErrorsListFile.close()
                continue
            if r.status_code ==200:
                mediaDoneList.append(line['vendorCode'])
                with open(r'D:\Python\WB\wipeAbraamayn\mediaDoneList.pkl', 'wb') as mediaDoneListFile:
