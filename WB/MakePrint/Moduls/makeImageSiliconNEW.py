...
from Folders import pathToPrintImageFolderAllSil, pathToMaskFolderSilicon, pathToDoneSiliconImageSilicon, pathToUploadFolderLocal
# import time
# import asyncio
# import aiofiles
...
def makeImageSiliconNEWMain(pathToMaskFolder, dirName, topPrint):
    ...
    pathToSaveDone =  pathToMaskFolder.replace(pathToMaskFolderSilicon, pathToDoneSiliconImageSilicon)
    pathToPrintImage = pathToPrintImageFolderAllSil # Путь откуда брать фотки для натяжки
    pathToImageDir = os.path.join(pathToMaskFolder, dirName)
    ...
    if maskImage == 0:
        ...
        return
    listImage = [x for x in os.listdir(pathToPrintImage) if x in topPrint]
    ...
    saveProcess = Thread(target=imageSaver, args=(queue, dirName, pathToSaveDone))
    saveProcess.start()
    ...
    for listImagePath in listForProcess:
        p = Process(target=createPrintImage, args=(pathToPrintImage, listImagePath, mask, maskImage, backImage, top_left, bottom_right, queue))
...
def imageSaver(queue, dirName, pathToSaveDone) :
